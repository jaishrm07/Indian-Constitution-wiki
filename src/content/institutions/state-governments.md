---
title: State Governments
slug: state-governments
institutionType: executive
summary: State governments are central to federal conflict, reservation policy, legislative standoffs, and many of the constitutional controversies that play out below the Union level.
role: State governments exercise executive power within their constitutional sphere and often become the primary actors in disputes over federal autonomy, public policy, and political legitimacy.
whyItMatters: A Constitution-focused site cannot explain Indian public law well if it treats all constitutional conflict as Union-level conflict.
constitutionalBasis:
  - Article 163 structures the relationship between the Governor and the elected Council of Ministers.
  - Article 174 matters when sessions, prorogation, or dissolution become constitutionally contested.
  - Part VI structures state executives and their relation to legislatures.
  - Part XI matters where state power overlaps with Union competence.
articles:
  - article-163
  - article-174
parts:
  - part-vi
  - part-xi
topics:
  - federalism
  - reservations
  - local-self-government
currentAffairs:
  - governor-state-standoffs
  - one-nation-one-election
  - reservation-and-sub-classification
sources:
  - constitution-of-india
  - supreme-court-of-india
  - press-information-bureau
---

## Why this institution matters

State governments are where federalism becomes concrete. Their conflicts with governors, Parliament, or the Union executive often define the real constitutional stakes of an issue.

## How to use this page

Use this institution page when a constitutional issue turns on state autonomy, executive power, or the interaction between state governance and the Union.
